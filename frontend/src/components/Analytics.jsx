...
const AnalyticsForBooks = () => {
    ...
    return (
        ...
        <>
        <h2 className="md:text-3xl text-xl font-zeyada text-white mb-4 text-center p-2"> Have a quick look at your rank , keep going {user?.name || "reader"} ! </h2>
        ...
        <div className="p-8 text-white flex items-center justify-center flex-col mx-auto m-4 rounded-lg shadow-md shadow-gray-200 w-full bg-gradient-to-b from-[#281d38] to-[#260d24] ">
                <Leaderboard  />
            <h2 className="md:text-2xl font-semibold mb-4 font-zeyada">Reading Analytics</h2>         
            <div className="mb-6">
                <label htmlFor="year" className="block text-black font-medium mb-2 text-white">Select Year</label>
                <select
                    id="year"
                    value={selectedYear}
                    onChange={handleYearChange}
                    className="p-2 border border-gray-300 bg-white text-black rounded text-sm"
                >
                    {data.years.map((year) => (
            ...
            <div className="chart-container md:w-1/2 p-4 mb-6 w-full">
                <h3 className="md:text-xl font-medium mb-2 text-sm">Books Read per Month</h3>
                <Bar data={booksReadData} />
            </div>

            <div className="summary-container mb-6">
                <h3 className="md:text-xl font-medium mb-2 text-sm">Average Rating</h3>
                <p>{data.average_rating[yearIndex] || 'N/A'}</p>

                <h3 className="md:text-xl font-medium mb-2 text-sm">Average Reading Speed (days)</h3>
